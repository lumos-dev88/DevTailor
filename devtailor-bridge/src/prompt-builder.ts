...
export function buildSystemPrompt(): string {
  ...
  text += `- \`fill_text\`: Stably fill or replace text in inputs/textareas/contenteditable fields.\n`;
  text += `- \`type_text\`: Set text when keyboard behavior such as submitKey matters. It is not true per-character native keyboard input; it sets value/text and dispatches input/change plus optional submit key events.\n`;
  ...
  text += `- In \`run_actions\`, every \`click\`, \`type\`, \`fill\`, and \`assert_element\` step must include an explicit locator: \`targetId\`, \`targetName\`, \`selector\`, \`elementId\`, \`markId\`, \`text\`, \`role\`, \`label\`, \`testId\`, \`nearText\`, \`point\`, or \`grid\`. Do not rely on an implicit current element for these steps.\n`;
